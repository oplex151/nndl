...
def generate_batch(batch_size, poems_vec, word_to_int):
    ...
    for i in range(n_chunk):
        ...
        x_data = np.full((batch_size, length), word_to_int[' '], dtype=np.int64)
...
def run_training():
    ...
    rnn_model = rnn_lstm.RNN_model(batch_sz = BATCH_SIZE,vocab_len = len(word_to_int) + 1 ,word_embedding = word_embedding ,embedding_dim= embedding_dim, lstm_hidden_dim=lstm_hidden_dim)

    optimizer=optim.RMSprop(rnn_model.parameters(), lr=0.01)

    loss_fun = torch.nn.NLLLoss()
    print("start  training")
    # rnn_model.load_state_dict(torch.load('./poem_generator_rnn_old_old'))  # if you have already trained your model you can load it by this line.
    ...
    for epoch in range(30):
        ...
        for batch in range(n_chunk):
            ...
            if batch % 20 ==0:
                torch.save(rnn_model.state_dict(), './poem_generator_rnn_old')
...
def gen_poem(begin_word):
    ...
    rnn_model.load_state_dict(torch.load('./poem_generator_rnn_old'))
